...
https://leetcode.cn/problems/surrounded-regions/solutions/2858956/xiao-zhou-ti-jie-130-bei-wei-rao-de-qu-y-byte/

思路：

- 构建一个大小为m*n的辅助矩阵`visit[][]`
  - 如果是延伸到边缘的`O`区域，则无法被围绕，因此这些`O`区域的`visit[][]`被标记为true 
  - 如果`board[i][j]`为`O`，且`visit[i][j]`为 False ，表示该`O`区域没有延伸到边缘，是要被替换为`X`的区域
- 遍历`board`矩阵的4个边缘，排除那些延伸到边缘导致无法被包围的`O`区域：
  - 如果遇到`X`就跳过 
  - 如果遇到`O`，将递归地其所在的整个`O`区域的`visit`置为 True
...
- 对于图而言，就两点区别：
  - 图有可能走回头路，所以需要用哈希表查重避免。而树只会一条路走到空节点，所以不需要哈希表。
  - 把复制树步骤的3.4改成对图节点邻接表的for循环
...
class Solution:
    ...
    def dfs(self, node: Optional['Node'], exist:dict):
        ...
        clone = Node(node.val, [])
        exist[node] = clone  # 创建哈希映射，将旧的节点与对应的新节点映射起来
        # 遍历旧节点的 neighbors ，并将其放入新节点的 neighbors 中
...
- 将链表分成两段，一段为有序链表，一段为待处理链表
- 每次从待处理链表中取出一个节点，将其放置到有序链表的正确位置
...
- 遍历`token`列表，
  - 当前字符为数字时，转为数字并入栈
  - 当前字符为操作符时，按序出栈操作数并进行操作，再将结果存回栈中
...
- 按题目要求操作，数组只有两种可能
  ...
  1. 完全升序。这种情况需要在整段数组里找，使用 一般的二分法即可

  2. 前大后小的两段，其内部升序。这种情况就**只用去后半段找**，使用 将`nums[mid]`与`nums[hight]`进行比较的二分法

- 因此，使用二分法，将`nums[mid]`与`nums[hight]`进行比较即可
